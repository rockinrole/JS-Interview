...
96. ### Ինչ է JavaScript-ում Symbol()-ի տարբեր կիրառությունները?

   
   
   **Symbol**-ը JavaScript-ում բազային տիպերից մեկն է, որը ստեղծում է եզակի և անփոփոխ արժեքներ։ Symbols-ը հաճախ օգտագործվում են որպես օբյեկտի հատկությունների բանալիներ՝ ապահովելու համար, որ այդ հատկությունները չեն բախվի այլ հատկությունների հետ։
   
   `Symbol`-ը ստեղծվում է `Symbol()` ֆունկցիայի միջոցով և կարող է ունենալ ընտրովի նկարագրություն՝ debugging նպատակներով։
...
---
   ...
   #### 1. **Օբյեկտի Հատկությունների Բանալիներ**
   Symbols-ը կարող են օգտագործվել որպես օբյեկտի հատկությունների եզակի բանալիներ՝ խուսափելով անվան բախումներից։
   
   ```javascript
   const sym = Symbol('uniqueKey');
   
   const obj = {
     [sym]: 'Value for the unique key'
   };
   
   console.log(obj[sym]); // "Value for the unique key"
   ```
   
   #### 2. **Մասնավոր Հատկությունների Ստեղծում**
   Symbols-ը ապահովում են "մասնավոր" հատկություններ, քանի որ դրանք չեն երևում սովորական հատկությունների իտրացիայի կամ `Object.keys`/`for...in` ցիկլերում։
   ...
   for (let key in obj) {
     console.log(key); // միայն "public"
   }
   ```
   
   #### 3. **Global Symbol Registry**
   `Symbol.for`-ը ստեղծում է կամ վերադարձնում է արդեն առկա `Symbol` արժեք `Global Symbol Registry`-ից։
   ...
   `Symbol.keyFor`-ը վերադարձնում է `Symbol.for`-ով ստեղծված sym-ի բանալին։
   
   ```javascript
   console.log(Symbol.keyFor(globalSym1)); // "globalKey"
   ```
   
   #### 4. **Ինտեգրացիա Ներկառուցված Օբյեկտների Հետ**
   JavaScript-ն ունի մի շարք "well-known symbols", որոնք թույլ են տալիս հարմարեցնել ստանդարտ օբյեկտների վարքագիծը։
   
   | **Symbol**               | **Նկարագրություն**                                                   |
   |--------------------------|---------------------------------------------------------------------|
   | `Symbol.iterator`        | Օբյեկտը դարձնում է իտրատոր։                                         |
   | `Symbol.toStringTag`     | Կարգավորում է `Object.prototype.toString` վերադարձվող արժեքը։       |
   | `Symbol.hasInstance`     | Կարգավորում է `instanceof` օպերատորի վարքագիծը։                     |
   | `Symbol.asyncIterator`   | Աջակցում է ասինխրոն իտրացիային։                                     |
   
   **Օրինակ `Symbol.iterator`-ի Հետ:**
   ```javascript
   const iterableObj = {
     data: [1, 2, 3],
     [Symbol.iterator]() {
   ...
   #### 5. **Կախվածությունների Կառավարում**
   Symbols-ը կարող է օգտագործվել որպես բանալիներ՝ կախվածությունները հստակ և մեկուսացված պահելու համար։
   ...
   class MyService {
     constructor() {
       this[dependency] = 'Important dependency';
     }
   ...
   console.log(service.getDependency()); // "Important dependency"
   ```
   
   #### 6. **Մեթոդների Օվեռայդ**
   Symbols-ը թույլ են տալիս փոխել օբյեկտների ստանդարտ մեթոդների վարքագիծը, օրինակ՝ `toString`։
...
---
   ...
   1. **Հատկությունների Անվտանգություն:**
      - Symbols-ը պաշտպանում է հատկությունները բախումներից։
   
   2. **Մասնավորություն:**
      - Օգտագործվում են "մասնավոր" հատկություններ ստեղծելու համար։
   
   3. **Հզոր Ինտեգրացիա:**
      - Աջակցում են ստանդարտ օբյեկտների վարքագծի հարմարեցմանը։
   
---
   
   #### Թերություններ
   
   1. **Մանրամասնությունը Բարդացնում Է Կոդը:**
      - Հատկապես նոր ծրագրավորողների համար Symbols-ը կարող են թվալ բարդ։
   
   2. **Համեմատման Դժվարություն:**
      - Symbols-ը միշտ եզակի են, ինչը կարող է խնդիր առաջացնել որոշ դեպքերում։
...
**[⬆ Back to Top](#բովանդակություն)**
   ...
   ---
97. ### Ինչ է JavaScript-ում Virtual DOM-ը?

   
   **Virtual DOM**-ը (Վիրտուալ DOM) JavaScript-ում ծրագրային մեխանիզմ է, որը օգտագործվում է օգտատերերի ինտերֆեյսների (UI) արագ և արդյունավետ թարմացման համար։
   
   Virtual DOM-ը հանդիսանում է պարզված պատճեն բրաուզերի իրական DOM-ի (Document Object Model), որն աշխատում է JavaScript-ի հիշողության մեջ։ Այն թույլ է տալիս կատարել փոփոխություններ նախքան դրանք կիրառելը իրական DOM-ի վրա՝ օպտիմալացնելով կատարողականությունը։
   
---
   
   #### Ինչպես է աշխատում Virtual DOM-ը
   
   1. **Ինտերֆեյսի Նախնական Պատկերացում (Initial Rendering):**
      - Երբ ծրագիրը բեռնվում է, ստեղծվում է Virtual DOM, որը հանդիսանում է հիշողության մեջ պահվող տվյալների կառուցվածք, որը ներկայացնում է իրական DOM-ը։
   
   2. **Փոփոխությունների Թարմացում (Updating):**
      - Երբ UI-ում փոփոխություններ են կատարվում, Virtual DOM-ը ստեղծում է նոր "ծառ" (tree), որը ներկայացնում է փոփոխված վիճակը։
   
   3. **Համեմատություն (Diffing):**
      - Virtual DOM-ը համեմատում է նախորդ և նոր "ծառերը"՝ գտնելու համար այն տարրերը, որոնք փոխվել են։
   
   4. **Patch-երի Կիրառում (Patching):**
      - Հաշվարկված փոփոխությունները (patch) կիրառվում են միայն այն հատվածների վրա, որոնք իրականում փոխվել են։
   
---
   
   #### Virtual DOM-ի Հիմնական Օրինակ
   ...
   #### Virtual DOM-ի Օգտագործմամբ (React Օրինակ)
   ```javascript
   import React from 'react';
   import ReactDOM from 'react-dom';
...
---
   ...
   #### Ինչու է Virtual DOM-ը Կարևոր
   
   1. **Արդյունավետ Կատարում:**
      - Արձագանքում է միայն անհրաժեշտ հատվածները՝ խուսափելով ավելորդ թարմացումներից։
   
   2. **Պարզ Կառավարում:**
      - Թույլ է տալիս ծրագրավորողներին աշխատել ավելի պարզ և ընթեռնելի կոդով։
   
   3. **Պլատֆորմային Անկախություն:**
      - Virtual DOM-ը կախված չէ բրաուզերի կոնկրետ իրականացման ձևից։
   
---
   
   #### Virtual DOM-ի Առավելությունները
   
   1. **Բարձր Արդյունավետություն:**
      - Կատարում է նվազագույն թարմացումներ իրական DOM-ում։
   
   2. **Մոդուլային Կառուցվածք:**
      - Աջակցում է UI-ի կառուցման կոմպոնենտային մոտեցմանը։
   
   3. **Նվազագույն Ռիսկեր:**
      - Թույլ է տալիս նախ փորձարկել փոփոխությունները հիշողության մեջ՝ նախքան դրանք կիրառելը։
   
   4. **Խաչաձև Պլատֆորմային Օգտագործում:**
      - Օգտագործվում է ինչպես վեբ, այնպես էլ մոբայլ հավելվածներում (օր.՝ React Native):
   
---
   
   #### Virtual DOM-ի Թերությունները
   
   1. **Հիշողության Օգտագործում:**
      - Virtual DOM-ը պահանջում է լրացուցիչ հիշողություն իր ծառի կառավարման համար։
   
   2. **Կոդի Բարդություն:**
      - Չնայած ծրագրավորման պարզեցմանը, ներքին մեխանիզմները բարդ են։
   
   3. **Չափի Խնդիրներ Փոքր Ծրագրերում:**
      - Փոքր ծրագրերում Virtual DOM-ի օգտագործումը կարող է ավելորդ լինել։
   
---
   
   #### Տարբերություն Virtual DOM-ի և Real DOM-ի միջև
   
   | **Հատկություն**        | **Real DOM**                       | **Virtual DOM**                     |
   |-------------------------|-------------------------------------|-------------------------------------|
   | **Կատարողականություն**  | Դանդաղ փոփոխություններ             | Արագ թարմացում                      |
   | **Հիշողության Օգտագործում** | Ցածր                              | Բարձր                              |
   | **Ներքին Իմպլեմենտացիա** | Միջանկյալ խավ չի պահանջվում         | Պահանջվում է միջանկյալ "ծառ"         |
   | **Մոտեցում**           | Աշխատում է ուղղակիորեն DOM-ի հետ   | Աշխատում է հիշողության մեջ          |
...
**[⬆ Back to Top](#բովանդակություն)**
   ...
   ---
98. ### Ինչ է JavaScript-ում Object.freeze()-ը?

   
   **`Object.freeze()`**-ը JavaScript-ի մեթոդ է, որը թույլ է տալիս "սառեցնել" օբյեկտը։ Սառեցված օբյեկտում փոփոխություններ կատարելը դառնում է անհնար, ինչը նշանակում է՝
   
   1. Օբյեկտի հատկությունները չեն կարող փոփոխվել։
   2. Նոր հատկություններ չեն կարող ավելացվել։
   3. Գոյություն ունեցող հատկությունները չեն կարող ջնջվել։
   4. Օբյեկտի հատկությունների կոնֆիգուրացիոն կարգավորումները չեն կարող փոփոխվել։
   
   Սա թույլ է տալիս պաշտպանել օբյեկտը սխալ կամ անցանկալի փոփոխություններից։
...
---
   ...
   Object.freeze(obj);
   ```
   - **`obj`**: Օբյեկտ, որը ցանկանում եք "սառեցնել"։
   
   **Վերադարձնվող Արժեք:** `Object.freeze()` վերադարձնում է "սառեցված" օբյեկտը։
...
---
   ...
   // Փորձում ենք փոփոխել հատկությունը
   person.age = 30;
   console.log(person.age); // 25 (փոփոխությունը չի կատարվում)
   
   // Փորձում ենք նոր հատկություն ավելացնել
   person.gender = 'female';
   console.log(person.gender); // undefined
   
   // Փորձում ենք ջնջել հատկությունը
   delete person.name;
...
---
   ...
   #### 3. Խորը Օբյեկտների Սառեցում
   **`Object.freeze`-ը գործում է միայն առաջին մակարդակի հատկությունների վրա։** Եթե օբյեկտում կան ներդրված օբյեկտներ, դրանք մնում են փոփոխելի։
   ...
   // Փորձում ենք փոփոխել ներդրված օբյեկտի հատկությունը
   user.details.age = 35;
   console.log(user.details.age); // 35 (ներդրված օբյեկտը փոփոխվում է)
   ```
   **Խորը սառեցման համար անհրաժեշտ է ձեռքով անցնել բոլոր մակարդակներով:**
   ...
   nestedObject.details.age = 35;
   console.log(nestedObject.details.age); // 28 (փոփոխությունը չի կատարվում)
   ```
...
---
   ...
   #### 1. Կայուն Տվյալների Ստեղծում
   Կայուն արժեքների ստեղծման համար, որոնք պետք է մնան անփոփոխ ամբողջ ծրագրում։
   ...
   // CONFIG.API_URL = 'https://newapi.example.com'; // Error (խորհուրդ չի տրվում)
   ```
   
   #### 2. Կարգավիճակի Պաշտպանություն
   Օգտագործվում է, երբ ծրագրում անհրաժեշտ է պաշտպանել տվյալների փոփոխություններից։
   
   ```javascript
   function createImmutableUser(name, age) {
     return Object.freeze({ name, age });
   }
   
   const user = createImmutableUser('Dave', 40);
   user.age = 45;
...
---
   ...
   1. **Կատարողականություն:**
      - `Object.freeze()` կարող է նվազեցնել կատարողականությունը, եթե օգտագործվում է հաճախ կամ մեծ օբյեկտների համար։
   
   2. **Եզակիություն:**
      - Սառեցված օբյեկտը չի կարող "հասարակացնել" այլ օբյեկտների հետ։
   
   3. **Խորը Սառեցում Չի Ապահովվում:**
      - Սառեցված օբյեկտի միայն առաջին մակարդակն է պաշտպանված։
   
---
   
   #### Թերություններ
   
   1. **Անհարմար Խորը Օբյեկտների Համար:**
      - Խորը սառեցման համար անհրաժեշտ է ձեռքով իրականացնել `deepFreeze` մեթոդը։
   
   2. **Անհրաժեշտություն Կառավարելու Վարքագիծը:**
      - `Object.freeze()` թույլ չի տալիս փոփոխություններ, նույնիսկ եթե դա անհրաժեշտ է։
...
**[⬆ Back to Top](#բովանդակություն)**
   ...
   ---
99. ### Ինչ է JavaScript-ում Object.seal()-ը?

   **`Object.seal()`**-ը JavaScript-ում մեթոդ է, որը "կնքում" է օբյեկտը։ "Կնքված" օբյեկտում նոր հատկություններ չեն կարող ավելացվել, և գոյություն ունեցող հատկությունները չեն կարող ջնջվել։ Այնուամենայնիվ, "կնքված" օբյեկտի գոյություն ունեցող հատկությունների արժեքները կարող են փոփոխվել, եթե դրանք writable են։
   
   Այս մեթոդը օգտակար է այն դեպքերում, երբ ցանկանում եք պաշտպանել օբյեկտը կառուցվածքային փոփոխություններից, սակայն թույլ տալ դրա հատկությունների արժեքների փոփոխություն։
...
---
   ...
   Object.seal(obj);
   ```
   - **`obj`**: Օբյեկտ, որը ցանկանում եք "կնքել"։
   
   **Վերադարձնվող Արժեք:** `Object.seal()` վերադարձնում է "կնքված" օբյեկտը։
...
---
   ...
   // Փորձում ենք փոփոխել հատկությունը
   person.age = 30;
   console.log(person.age); // 30 (փոփոխությունը կատարվում է)
   
   // Փորձում ենք նոր հատկություն ավելացնել
   person.gender = 'female';
   console.log(person.gender); // undefined (ավելացումը չի կատարվում)
   
   // Փորձում ենք ջնջել հատկությունը
   delete person.name;
   console.log(person.name); // "Alice" (ջնջումը չի կատարվում)
   ```
   
---
   
   #### 2. Նույն արժեքի վերադարձ
   `Object.seal()`-ը վերադարձնում է նույն օբյեկտը, որը փոխանցվել է։
   
   ```javascript
   const obj = { key: 'value' };
   const sealedObj = Object.seal(obj);
   
   console.log(sealedObj === obj); // true
   ```
   
---
   
   #### Ինչպես Ստուգել `Object.seal()`-ի Վիճակը
   
   Կարող եք օգտագործել `Object.isSealed()` մեթոդը՝ ստուգելու համար, արդյոք օբյեկտը "կնքված" է։
...
---
   ...
   #### Տարբերություն `Object.seal()`-ի և `Object.freeze()`-ի Միջև
   
   | **Հատկանիշ**             | **`Object.seal()`**                     | **`Object.freeze()`**                   |
   |--------------------------|----------------------------------------|----------------------------------------|
   | **Նոր հատկություններ**    | Չի թույլատրվում ավելացնել               | Չի թույլատրվում ավելացնել               |
   | **Հատկությունների ջնջում**| Չի թույլատրվում                         | Չի թույլատրվում                         |
   | **Գոյություն ունեցող արժեքների փոփոխում** | Թույլատրվում, եթե writable են            | Չի թույլատրվում                         |
   | **Կոնֆիգուրացիա**         | Չի կարող փոխվել                          | Չի կարող փոխվել                          |
   
---
   
   #### Խորը Կնքում Չի Ապահովվում
   
   **`Object.seal()`-ը գործում է միայն օբյեկտի առաջին մակարդակի հատկությունների վրա։** Եթե օբյեկտը պարունակում է ներդրված օբյեկտներ, դրանք կմնան փոփոխելի։
   ...
   // Փորձում ենք փոփոխել ներդրված օբյեկտի հատկությունը
   user.details.age = 35;
   console.log(user.details.age); // 35 (փոփոխությունը կատարվում է)
   ```
   
   Խորը կնքում իրականացնելու համար պետք է օգտագործել ձեռքով անցում բոլոր հատկություններով.
   ...
   deepSeal(nestedObject);
   
   nestedObject.details.age = 35; // Այս դեպքում փոփոխություն չի կատարվի։
   console.log(nestedObject.details.age); // 28
   ```
...
---
   ...
   1. **Կառուցվածքային Պաշտպանություն:**
      - Օգտագործվում է, երբ ցանկանում եք թույլ տալ միայն արժեքների փոփոխություն՝ արգելելով նոր հատկությունների ավելացումը կամ առկա հատկությունների ջնջումը։
   
   2. **Կարգավիճակի Կայունացում:**
      - Օգտագործվում է կայուն տվյալների ապահովման համար։
   ...
   CONFIG.TIMEOUT = 10000; // Թույլատրվում է
   CONFIG.newKey = 'value'; // Չի թույլատրվում
   ```
...
---
   ...
   1. **Կառուցվածքի Պաշտպանություն:**
      - Արգելում է նոր հատկությունների ավելացումը կամ ջնջումը։
   
   2. **Գոյություն ունեցող հատկությունների փոփոխման Թույլտվություն:**
      - Թույլ է տալիս փոփոխել արժեքները՝ առանց կառուցվածքը փոխելու։
   
---
   
   #### Թերություններ
   
   1. **Խորը Կնքում Չի Ապահովվում:**
      - Ներդրված օբյեկտները մնում են փոփոխելի։
   
   2. **Կառավարում:**
      - Կոդը կարող է բարդանալ, եթե պահանջվում է խորը կնքում։
...
**[⬆ Back to Top](#բովանդակություն)**
   ...
   ---
100. ### Ինչ է JavaScript-ում Function Composition-ը?

   **Function Composition**-ը JavaScript-ում տեխնիկա է, որն օգտագործվում է երկու կամ ավելի ֆունկցիաների համակցման համար՝ ստեղծելով նոր ֆունկցիա։
   
   Այս մոտեցումը կարևոր է ֆունկցիոնալ ծրագրավորման մեջ և թույլ է տալիս ծրագրավորողներին կառուցել հոսք, որտեղ տվյալները փոխանցվում են ֆունկցիաների միջով՝ պահպանելով պարզություն և վերաօգտագործելիություն։
   
   #### Հիմնական Սինտաքս
   
   ```javascript
   const composedFunction = (input) => func2(func1(input));
   ```
   
   Այս օրինակում՝
   - `func1`-ը առաջին ֆունկցիան է, որը գործում է ներմուծված տվյալների վրա։
   - `func2`-ը ֆունկցիան է, որը կիրառում է արդյունքը `func1`-ի վրա։
...
---
   ...
   console.log(composedFunction(5)); // 21 ((5 + 2) * 3)
   ```
   
---
   
   #### 2. Օգտագործելով Ֆունկցիաների Ցանկ
   ...
   const composed = compose(subtract, square, divide);
   
   console.log(composed(10)); // 20 ((10 - 1)^2 / 2)
   ```
   
---
   
   #### 3. Կիրառում Պրակտիկայում
   
   #### Օրինակ՝ Հոսքի Ստեղծում Տեքստի Մշակման Համար
   ...
   const processText = compose(trim, toLowerCase, splitWords);
...
---
   ...
   #### Function Composition-ի Առավելությունները
   
   1. **Կոդի Վերաօգտագործելիություն:**
      - Փոքր ֆունկցիաները կարող են օգտագործվել մի քանի վայրերում՝ առանց կրկնօրինակելու։
   
   2. **Կոդի Կառուցվածք:**
      - Թույլ է տալիս գրել կոդ, որը ավելի ընթեռնելի է և մոդուլային։
   
   3. **Թեստավորման Հեշտություն:**
      - Յուրաքանչյուր ֆունկցիա կարելի է առանձին թեստավորել՝ ապահովելով ծրագրի հուսալիությունը։
   
   4. **Ֆունկցիոնալ Ծրագրավորման Խթանում:**
      - Function Composition-ը հիմնարար տարր է ֆունկցիոնալ ծրագրավորման մեջ, ինչը օգնում է ստեղծել պարզ, կանխատեսելի և ընդարձակելի կոդ։
   
---
   
   #### Function Composition-ի Սահմանափակումներ
   
   1. **Դանդաղ Կատարում Թույլ Կոդերի Դեպքում:**
      - Եթե ֆունկցիաները լավ օպտիմալացված չեն, կատարման ժամանակը կարող է աճել։
   
   2. **Դժվար Պատկերացում:**
      - Սկսնակների համար կարող է բարդ թվալ հասկանալ, թե ինչպես են տվյալները հոսում ֆունկցիաների միջով։
   
---
   
   #### Function Composition-ի Համեմատ Functors-ի Հետ
   
   Function Composition-ը հաճախ համեմատվում է functors-ի հետ, սակայն դրանք տարբեր են։
   
   | **Հատկանիշ**            | **Function Composition**                           | **Functor**                                         |
   |--------------------------|---------------------------------------------------|----------------------------------------------------|
   | **Նպատակ**              | Համակարգում է ֆունկցիաների աշխատանքը               | Աշխատում է տվյալների վրա՝ թաքցնելով բարդությունները |
   | **Օգտագործում**          | Մեկ ֆունկցիայի արդյունքը փոխանցվում է հաջորդին     | Թույլ է տալիս փոփոխություններ տվյալների վրա       |
